locals {
  secrets                  = read_terragrunt_config(find_in_parent_folders("secrets/extras/buddy.hcl"))
  enabled                  = false
  buddy_region             = "EU" # OPTIONS "US" or "EU"
  buddy_github_token       = local.secrets.locals.buddy_github_token
  buddy_personal_token     = local.secrets.locals.buddy_personal_token
