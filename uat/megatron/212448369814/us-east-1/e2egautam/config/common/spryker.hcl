locals {
  ...
  spryker_ecs_services      = ["yves", "boffice", "backgw", "glue", "frontend", "rabbitmq", "blackfire", "jenkins"] // full list of enabled ecs services
  ...
  services_to_restart       = ["yves", "boffice", "backgw", "glue", "frontend"]
  spryker_ecr_repos         = ["yves", "boffice", "backgw", "glue", "frontend", "rabbitmq", "blackfire", "jenkins", "pipeline"] # pipeline repository is obligatory starting from 6.0, docker/sdk produces image inside of it
  region                    = "us-east-1"
  project_owner             = "uat"
  project_name              = "uat-e2egautam"
  customer_project          = "megatron"
  aws_account_id            = "212448369814"
  env_type                  = "e2egautam"
  route53_zone_domain       = "uat-e2egautam.cloud.spryker.toys"
  scheduler_fqdn            = "jenkins.uat-e2egautam.cloud.spryker.toys"
  scheduler_type            = "ecs"
  ...
  deploy_file               = "deploy.uat-e2egautam.yml"
  aop_authentication        = local.secrets.locals.aop_authentication
  aws_account_environment   = "internal" // only use "production" or "non-production" for customers and "internal" for sandboxes
  released_version          = "change_me" //git tagged version. ie. 7.0.0
  tags = {
