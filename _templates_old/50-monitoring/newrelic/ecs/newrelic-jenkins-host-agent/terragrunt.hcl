...
skip = local.spryker.locals.scheduler_type != "ecs" || !local.newrelic.locals.newrelic_integration.enable_production_mode

terraform {
  source = "git@github.com:spryker-projects/tfcloud-modules.git//refactored/aws_ecs_service?ref=v8.0.0"
}
...
inputs = {
  ...
  execution_role_arn                 = local.spryker.locals.scheduler_type == "ecs" && local.newrelic.locals.newrelic_integration.enable_production_mode == true ? dependency.iam_ecs_service.outputs.aws_iam_role_arn[local.spryker_service.locals.service_name] : null
